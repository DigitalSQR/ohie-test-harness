...
const EditQuestion = () => {
  ...
  const [changesMadeToEditOption, setChangesMadeToEditOption] = useState<boolean>(false);
  ...
  const props: UploadProps = {
    ...
    customRequest: (options) => {
      if(options.file){
        ...
        .then(async (response) => {
          const base64Image = await DocumentAPI.base64Document(response.id,response.name);
          let imageData = {
              name: response.name,
              status: 'done',
              url: base64Image,
  ...
  useEffect(() => {
    setChangesMadeToEditOption(JSON.stringify(editedTestcaseOptions[currentAccordionIndex]) != JSON.stringify(initialTestcaseOptions[currentAccordionIndex]));
  },[editedTestcaseOptions]);

  useEffect(() => {
    setChangesMade(editedQuestion !== initialQuestion || editedQuestionType !== initialQuestionType);
  ...
  const handleSaveQuestion = async () => {
    try {
      showLoader();
      var patch = [{op: "replace", path: "/name" , value : editedQuestion}
  ...
  const handleSelectAccordionItem = (index) => {
    ...
    if(currentAccordionIndex){
      ...
      updatedOptions[currentAccordionIndex] = initialTestcaseOptions[currentAccordionIndex];
  ...
  const handleIsCorrectChange = (index, e) => {
    const updatedOptions = [...editedTestcaseOptions];
    updatedOptions[index] = { ...updatedOptions[index], success: e.target.checked };
    setEditedTestcaseOptions(updatedOptions);
  }

  const handleSaveOption = (index) => {
    let optionToUpdate = editedTestcaseOptions[index];
    var patch = [{op: "replace", path: "/name" , value : optionToUpdate.name}
  ...
  const changeOptionState = (index) => {
    ...
    if (initialTestcaseOptions[index].state === 'testcase.option.status.active') {
      confirmStateChange();
    } else {
      handleStateChange(newState);
    }
  ...
  return (
    currentTestcase && <div id="wrapper">
              ...
              <div className="form-group mb-3">
                ...
                <div className="form-check">
                  <input
                    ...
                    onChange={(e) => handleQuestionTypeChange(e.target.value)}
                  />
                  <label className="form-check-label" htmlFor="singleSelect">
                    Single Select
                  </label>
                </div>
              </div>
              <button disabled={!changesMade} onClick={handleSaveQuestion} type="button" className="btn btn-primary mt-3">Save</button>
            </form>
          </div>
          ...
          {editedTestcaseOptions.map((option, index) => (
            <div key={index}>
              <Accordion.Item eventKey={index.toString()}>
                <Accordion.Header> 
                  {initialTestcaseOptions[index].success ? <i className="bi bi-check-circle-fill me-2 text-success"></i> : <i className="bi bi-x-circle-fill me-2 text-danger"></i>} 
                  
                  {initialTestcaseOptions[index].name}</Accordion.Header>
                <Accordion.Body>
                      ...
                      <div className="col-auto">
                        ...
                        <span className="form-check form-switch">
                          <Switch
                            ...
                            checked={option?.state === "testcase.option.status.active"}
