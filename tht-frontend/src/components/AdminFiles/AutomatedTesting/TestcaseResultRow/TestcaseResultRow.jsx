...
//   Holds data about the testcaseResult.
    ...
    return (
      <div className={"collapse " + toggleClass + " expanded-row"}>
        {testcaseResult?.hasSystemError ? (
          ...
          <p dangerouslySetInnerHTML={{ __html: messageSanitize }}></p>
        )}
      </div>
    );
  };
...
// Effect to subscribe to updates on the testcaseResult via WebSocket using the stompClient.
  ...
  return (
    testcaseResult && (
      <Fragment>
        <tr
          key={`testcase-result-${testcaseResult?.id}`}
          className="testcase-result-row"
        >
          {displayTestName()}
