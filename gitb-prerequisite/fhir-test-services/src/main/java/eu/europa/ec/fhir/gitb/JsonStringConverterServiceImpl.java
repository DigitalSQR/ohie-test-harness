 ...
 */
@Component
public class JsonStringConverterServiceImpl implements MessagingService {
     ...
     * Logger.
     */
    private static final Logger LOG = LoggerFactory.getLogger(JsonStringConverterServiceImpl.class);
    @Resource
    ...
    public SendResponse send(SendRequest sendRequest) {
        ...
        if (JSON_STRING.equals(bodyType)) {
            //If Body is JsonString then convert it to AnyConnect Type
            report.getContext().getItem().add(
                    utils.getAnyConnectForString(
                            "convertedBody",
                            body,
                            null)
            );
        } else {
            //If Body is not JsonString then convert it to JsonString
            report.getContext().getItem().add(
                utils.createAnyContentSimple("convertedBody", body, ValueEmbeddingEnumeration.STRING, null)
            );
        }
