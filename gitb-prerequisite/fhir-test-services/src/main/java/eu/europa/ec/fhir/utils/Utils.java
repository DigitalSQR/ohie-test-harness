...
public class Utils {
    ...
    public String getPrettyStringAndFixType(AnyContent anyContent, int i) {
        StringBuffer stringBuffer = new StringBuffer();
        for (int j = 0; j < i; j++) {
            stringBuffer.append("\t");
        }
        stringBuffer.append(anyContent.getName()).append(" [").append(anyContent.getType()).append("] : ");
        if (anyContent.getType().startsWith("list")) {
            anyContent.setType("list");
            stringBuffer.append("[\n");
            i++;
            for (AnyContent childAnyContent : anyContent.getItem()) {
                stringBuffer.append(getPrettyStringAndFixType(childAnyContent, i));
                for (int j = 0; j < i; j++) {
                    stringBuffer.append("\t");
                }
                stringBuffer.append(",\n");
            }
            if(!CollectionUtils.isEmpty(anyContent.getItem())) {
                for (int j = 0; j <= i; j++) {
            ...
            stringBuffer.append("]\n");
        } else if (anyContent.getType().startsWith("map")) {
            anyContent.setType("map");
            stringBuffer.append("{\n");
            i++;
            for (AnyContent childAnyContent : anyContent.getItem()) {
                stringBuffer.append(getPrettyStringAndFixType(childAnyContent, i));
            ...
            stringBuffer.append(anyContent.getValue()).append("\n");
        }
    ...
    public JsonElement getJsonElementForAnyConnect(AnyContent anyConnect) {
        if (anyConnect.getType().startsWith("map")) {
            return getJsonObjectForAnyConnects(anyConnect.getItem());
        } else if (anyConnect.getType().startsWith("list")) {
            return getJsonArrayForAnyConnects(anyConnect.getItem());
    ...
    public void assignAnyContent(AnyContent assignTo, AnyContent value, String path) {
        List<String> variables = extractComponents(path);
        for(String variable: variables) {
            boolean found = false;
            for(AnyContent anyContent: assignTo.getItem()) {
                if(anyContent.getName().equals(variable)) {
                    assignTo = anyContent;
                    found = true;
                    break;
                }
            ...
            if(!found) {
                AnyContent requestItem = new AnyContent();
                requestItem.setName(variable);
                requestItem.setType("map");
                if(isNumber(variable)) {
                    assignTo.setType("list");
                }
                assignTo.getItem().add(requestItem);
                assignTo = requestItem;
            }
        }
        assignTo.setType(value.getType());
        for(AnyContent anyContent: value.getItem()) {
            assignTo.getItem().add(anyContent);
        }
    }

    public static boolean isNumber(String input) {
        // Regular expression to match numbers from 0 to 9
        String regex = "[0-9]+";
    ...
    public static List<String> extractComponents(String input) {
        ...
        // Extract components and add them to the list
        while (matcher.find()) {
