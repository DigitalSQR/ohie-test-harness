    ...
    public ValidationResultInfo test(Map<String, IGenericClient> iGenericClientMap,
        ...
        try {
            ...
            if (!operativeNoteOneOutcome.getCreated()) {
                LOGGER.error(testCaseName + "Testcase Failed when creating Composition");
                return new ValidationResultInfo(testCaseName, ErrorLevel.ERROR, "Failed to create operative Composition");
            }
            ...
            if (!progressNoteOneOutcome.getCreated()) {
                LOGGER.error(testCaseName + "Testcase Failed when creating Composition");
                return new ValidationResultInfo(testCaseName, ErrorLevel.ERROR, "Failed to create progress Composition");
            }
            ...
            if (!dischargeSummaryOneOutcome.getCreated()) {
                LOGGER.error(testCaseName + "Testcase Failed when creating Composition");
                return new ValidationResultInfo(testCaseName, ErrorLevel.ERROR, "Failed to create Discharge Composition");
            }
            ...
            if ((!bundle.hasEntry()) || actualTotal != expectedTotal) {
                ...
                return new ValidationResultInfo(testCaseName, ErrorLevel.ERROR, "Composition has no entry or not as expected entry");
            }
            for (Bundle.BundleEntryComponent entry : bundle.getEntry()) {
                ...
                if (resource instanceof Composition composition) {
                    ...
                    if (!title.equals("Admission Note") && !title.equals("Operative Note") && !title.equals("Progress Note") && !title.equals("Discharge Summary")) {
                        ...
                        return new ValidationResultInfo(testCaseName, ErrorLevel.ERROR, "Test case failed because of wrong title");
                    }
                    ...
                    if (!subjectRef.getReference().equals(patientReference)) {
                        ...
                        return new ValidationResultInfo(testCaseName, ErrorLevel.ERROR, "Test case failed because of Invalid subject reference" + subjectRef.getReference());
                    }
                    ...
                    if (!authorFound) {
                        ...
                        return new ValidationResultInfo(testCaseName, ErrorLevel.ERROR, "Test case failed because Author reference not found");
                    }
                } else {
                    LOGGER.error(testCaseName + "Test case failed because Resource type is not of Composition");
                    return new ValidationResultInfo(testCaseName, ErrorLevel.ERROR, "Test case failed because Resource type is not of Composition");
                }
            }
            LOGGER.info(testCaseName + "Testcase successfully passed!");
            return new ValidationResultInfo(testCaseName, ErrorLevel.OK, "Passed");
        } catch (Exception ex) {
            LOGGER.error("caught OperationFailedException in SHRF5TestCase2 ", ex);
