...
public class SHRF5TestCase1 implements TestCase {
    ...
    public ValidationResultInfo test(Map<String, IGenericClient> iGenericClientMap,
        ...
        try {
            ...
            if (Boolean.FALSE.equals(admissionNoteOutcome.getCreated())) {
                ...
                for (Bundle.BundleEntryComponent entry : compositionResults.getEntry()) {
                    if (entry.getResource() instanceof Composition composition) {
                        ...
                        if (!(compositionId.equals(originalCompositionId))) {
                            LOGGER.error("{} Test case failed because of different composition Id in Bundle", testCaseName);
                        ...
                        if (authors == null || authors.isEmpty()) {
                            LOGGER.error("{} Test case failed because no Author found in Bundle", testCaseName);
                        ...
                        if (!(authorReference.equals(practitionerReference))) {
                            LOGGER.error("{} Test case failed because of different Practitioner reference in Bundle", testCaseName);
                            return new ValidationResultInfo(ErrorLevel.ERROR, "Different Practitioner reference");
                        }
                        if (!(title.equals(admissionNote.getTitle()))) {
                            LOGGER.error("{} Test case failed because of different title in Admission Note in Bundle", testCaseName);
            ...
            LOGGER.error("{} Test case failed because Composition has no Entry", testCaseName);
